...
#ifdef _WIN32
    #define NOMINMAX  // 防止 windows.h 污染 std::min/max
#ifdef min
    #undef min
#endif
#ifdef max
...
class YOLO_3D_seg {
public:
    explicit YOLO_3D_seg(const std::string& engine_file_path);
    ~YOLO_3D_seg();
...
YOLO_3D_seg::YOLO_3D_seg(const std::string& engine_file_path) {
    ...
    for (int i = 0; i < this->num_bindings; ++i) {
        ...
        nvinfer1::DataType dtype = this->engine->getTensorDataType(tensorName);
        ...
        // bool IsInput = engine->getTensorIOMode(tensorName);
        bool IsInput = (engine->getTensorIOMode(tensorName) ==
        ...
        if (IsInput) {
            ...
            dims = this->engine->getProfileShape(
                    tensorName, 0, nvinfer1::OptProfileSelector::kMAX);
            binding.size = get_size_by_dims(dims);
            binding.dims = dims;
            this->input_bindings.push_back(binding);
            // set max opt shape
            // this->context->setBindingDimensions(i, dims);
            this->context->setInputShape(tensorName, dims);
        } else {
            // dims = this->context->getBindingDimensions(i);
            dims = this->context->getTensorShape(tensorName);
            binding.size = get_size_by_dims(dims);
            binding.dims = dims;
...
void YOLO_3D_seg::letterbox(const cv::Mat& image,
                            cv::Mat& out,
                            cv::Size& size) {
...
void YOLO_3D_seg::copy_from_Mat(const cv::Mat& image) {
    cv::Mat nchw;
    ...
    //         0, nvinfer1::Dims{4, {1, 3, height, width}});
    // this->context->setInputShape(0, nvinfer1::Dims{4, {1, 3, height,
    // width}});
...
void YOLO_3D_seg::copy_from_Mat(const cv::Mat& image, cv::Size& size) {
    cv::Mat nchw;
    this->letterbox(image, nchw, size);
    // this->context->setBindingDimensions(
...
void YOLO_3D_seg::infer() {
    // this->context->enqueueV2(this->device_ptrs.data(), this->stream,
    // nullptr);

    // Use enqueueV3
    for (int i = 0; i < this->num_inputs; ++i) {
        const char* tensorName = this->input_bindings[i].name.c_str();
        if (!this->context->setTensorAddress(tensorName,
                                             this->device_ptrs[i])) {
            std::cerr << "Error: Failed to set input tensor address for "
                      << tensorName << std::endl;
        }
    }

    for (int i = 0; i < this->num_outputs; ++i) {
        const char* tensorName = this->output_bindings[i].name.c_str();
        if (!this->context->setTensorAddress(
                    tensorName, this->device_ptrs[i + this->num_inputs])) {
            std::cerr << "Error: Failed to set output tensor address for "
                      << tensorName << std::endl;
        }
    }
...
void YOLO_3D_seg::postprocess(std::vector<Object>& objs,
                              float score_thres,
                              float iou_thres,
                              int topk,
                              int seg_channels,
    ...
    for (int i = 0; i < num_anchors; i++) {
        ...
        if (score > score_thres) {
            ...
            cv::Mat mask_conf =
                    cv::Mat(1, seg_channels, CV_32F, mask_confs_ptr);
    ...
    if (masks.empty()) {
        ...
        // std::cout << "matmulRes.size():" << matmulRes.size() << std::endl;
        // std::cout << "matmulRes.size():" << maskMat.size() << std::endl;
        std::vector<cv::Mat> maskChannels;
        cv::split(maskMat, maskChannels);
        int scale_dw = dw / input_w * seg_w;
        int scale_dh = dh / input_h * seg_h;

        cv::Rect roi(scale_dw, scale_dh, seg_w - 2 * scale_dw,
                     seg_h - 2 * scale_dh);
...
void YOLO_3D_seg::draw_objects(
    ...
    for (auto& obj : objs) {
        int idx = obj.label;
        cv::Scalar color =
                cv::Scalar(COLORS[idx][0], COLORS[idx][1], COLORS[idx][2]);
        cv::Scalar mask_color =
                cv::Scalar(MASK_COLORS[idx % 20][0], MASK_COLORS[idx % 20][1],
