//
// This file is part of Open3D project
//
// ----------------------------------------------------------------------------
...
std::tuple<bool, Eigen::VectorXd> SolveLinearSystemPSD(
    ...
    if (check_symmetric && !A.isApprox(A.transpose())) {
        LOG_WARN("check_symmetric failed, empty vector will be returned");
    ...
    if (check_det) {
        ...
        if (fabs(det) < 1e-6 || std::isnan(det) || std::isinf(det)) {
            LOG_WARN("check_det failed, empty vector will be returned");
            return std::make_tuple(false, Eigen::VectorXd::Zero(b.rows()));
        }
    }
    ...
    if (check_psd) {
        ...
        if (A_llt.info() == Eigen::NumericalIssue) {
            LOG_WARN("check_psd failed, empty vector will be returned");
            return std::make_tuple(false, Eigen::VectorXd::Zero(b.rows()));
        }
    }
    ...
    if (prefer_sparse) {
        ...
        if (A_chol.info() == Eigen::Success) {
            ...
            if (A_chol.info() == Eigen::Success) {
                ...
                LOG_WARN("Cholesky solve failed, switched to dense solver");
            }
        } else {
            LOG_WARN("Cholesky decompose failed, switched to dense solver");
        }
    }
...
#pragma omp critical(ComputeJTJandJTr)
    ...
    if (verbose) {
        LOG_DEBUG("Residual : {:.2e} (# of elements : {:d})",
